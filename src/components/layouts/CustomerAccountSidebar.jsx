...
import {
  ...
  AiOutlineEnvironment, // Location pin icon (Addresses)
} from "react-icons/ai";
import { BsRobot, BsTicketDetailed } from "react-icons/bs"; // Robot icon (AI Support) and ticket icon (Support Tickets)
import { MdOutlineAssignmentReturn, MdVerified } from "react-icons/md"; // Return/assignment icon (Returns), Verified badge icon
...
const NAV_ITEMS = [
  ...
  {
    label: "Addresses", // Text shown for this nav item
    icon: <AiOutlineEnvironment className="w-5 h-5" />, // Icon for this nav item
    route: ROUTES.ACCOUNT_ADDRESSES, // Route this item navigates to
    badgeKey: null, // No badge count for this item
  },
  {
...
const CustomerAccountSidebar = () => {
  ...
  const { data: ordersData } = useQuery({
    ...
    queryFn: ({ signal }) => getMyOrders(undefined, signal), // Function that fetches the user's orders from the API
  ...
  const { data: notificationsData } = useQuery({
    ...
    queryFn: ({ signal }) => getNotifications(undefined, signal), // Function that fetches notifications from the API
    staleTime: 1000 * 60 * 2, // Data is considered fresh for 2 minutes before refetching
  });
  ...
  const { data: profileData } = useQuery({
    ...
    queryFn: ({ signal }) => getMyProfile(signal), // Function that fetches the user's profile from the API
