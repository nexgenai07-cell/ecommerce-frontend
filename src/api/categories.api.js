...
// an actual File object is present.
//
// UPDATED (Sep 2026, API 24/26/27.1 backend fix): the backend used to
// wrongly block creating (or renaming into) a name that matched a
// SOFT-DELETED category, even though that name was no longer visible
// anywhere — this is now fixed, soft-deleted names are correctly
// treated as available again. No shape or frontend logic change was
// needed here; the existing "already exists" duplicate-name check
// (API 24.1 below) now simply returns the correct answer.
export const createCategory = (data, signal) => {
  // No image attached — send plain JSON, simplest case, no need for
  // FormData overhead
  if (!(data.image instanceof File)) {
