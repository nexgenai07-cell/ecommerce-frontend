...
// exportReport — `type: "social_posts"` is a CONFIRMED accepted value
// (API 99, v5.9). NOTE: the backend does not yet accept status/platform/
// search as extra filters for this type (only start_date/end_date, which
// this page has no UI for either) — so, same as Discounts/Complaints/
// Inventory, the export currently always returns every post regardless
// of what's selected here. See ExportData.jsx / API 99 docs — flagged
// as a backend follow-up, not something this page can fix alone.
...
const PostsList = () => {
  ...
  // --------------------------------------------------
  // EXPORT — API 99, type=social_posts. downloadExportCsv() reads the
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (success) {
        showSuccess("Posts exported.");
      } else {
        showError(message || "Failed to export posts. Please try again.");
      }
    } finally {
